# ⚙️ mcp-devtools: 基于SSE的多功能开发工具MCP服务器
  ...
  - 🔧 `mcp-devtools` 提供了一套全面的开发工具集: [ℹ️ 可用工具](#%E2%84%B9%EF%B8%8F-%E5%8F%AF%E7%94%A8%E5%B7%A5%E5%85%B7)
      - 🎋 Git 管理操作 (`git_status`, `git_stage_and_commit`, `git_diff`, `git_log`, `git_show`)
      - 📁 Git 文件操作 (`git_read_file`)
...
<details>
<summary> <h4> ℹ️ 推荐的 Aider 配置 </h4> </summary>

- 在仓库根目录（优先）或用户主目录（`~/.aider.conf.yml`）创建或复制 `.aider.conf.yml`。
- 从仓库根目录中的 `.aider.conf.yml` 文件作为模板开始，并根据需要进行调整（模型、API 密钥、自动提交行为、包含/排除等）。
- 服务器会自动加载工作区中的 `.aider.conf.yml`；将其放在仓库根目录或 HOME 即可满足大多数工作流。
- 参阅 [📄 官方 Aider 文档](https://aider.chat/docs/config.html) 以了解每个选项的详细说明。

</details>

## 🤖 `ai_edit` 工作流

`ai_edit` 工具提供了一种使用自然语言进行代码更改的强大方法。它不再自动提交更改，而是将更改应用到您的工作目录，并提供结构化报告供您审查。

### 工作原理

1.  **委派任务:** 使用清晰的指令和目标文件调用 `ai_edit`。
2.  **接收报告:** 该工具返回一份报告，其中包含：
    *   **Aider 的计划:** AI 将采取的方法。
    *   **应用的更改 (Diff):** 对您的文件所做的确切更改。
    *   **后续步骤:** 有关如何手动审查、暂存和提交更改的说明。
3.  **审查和提交:** 您完全掌控。审查差异，如果批准，请使用 `git_stage_and_commit` 工具暂存并提交更改。
...
**缓解措施:**
  ...
  * 🔨 `write_to_file` 工具在编辑时与 `tsc` (TypeScript 编译器) 动态集成，用于对 `.js`、`.mjs` 和 `.ts` 文件进行条件类型检查。`tsc --noEmit --allowJs` 的输出作为工具响应的一部分提供。AI 助手应解析此输出以检测任何编译器错误，并且如果报告了错误，则*不应继续执行进一步的操作*，这表明写入的代码存在问题。
...
### `git_show`
  ...
  - **描述:** 显示特定提交或提交范围（A..B 或 A...B）的元数据（作者、日期、消息）和差异。这允许检查特定提交或提交范围引入的更改。可选择按路径过滤或仅显示元数据/差异。
  - **输入模式:**
    ```json
    {
      "type": "object",
