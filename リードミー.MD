# ⚙️ mcp-devtools: SSE経由の多機能開発ツールMCPサーバー
  ...
  - 🔧 `mcp-devtools` は、包括的な開発ツール群を提供します: [ℹ️ 利用可能なツール](#%E2%84%B9%EF%B8%8F-%E5%88%A9%E7%94%A8%E5%8F%AF%E8%83%BD%E3%81%AA%E3%83%84%E3%83%BC%E3%83%AB)
      - 🎋 Git管理操作 (`git_status`, `git_stage_and_commit`, `git_diff`, `git_log`, `git_branch`, `git_show`)
      - 📁 Gitファイル操作 (`git_read_file`)
...
`ai_edit` ツールは、自然言語を使ってコード変更を行う強力な方法を提供します。変更は自動的にコミットされなくなりました。代わりに、ワーキングディレクトリに変更を適用し、レビュー用の構造化されたレポートを提供します。

### 仕組み

1.  **タスクの委任:** `ai_edit` を明確な指示と対象ファイルを指定して呼び出します。
2.  **レポートの受信:** ツールは以下の内容を含むレポートを返します：
    *   **Aiderの計画:** AIが取るアプローチ。
    *   **適用された変更（Diff）:** ファイルに加えられた正確な変更。
    *   **次のステップ:** 変更を手動でレビュー、ステージング、コミットするための指示。
3.  **レビューとコミット:** あなたが完全にコントロールします。差分を確認し、承認した場合は `git_stage_and_commit` ツールを使って変更をステージングし、コミットします。
...
**緩和策:**
  ...
  * 🔨 `write_to_file` ツールは、編集時に `.js`、`.mjs`、`.ts` ファイルの条件付き型チェックのために `tsc`（TypeScriptコンパイラ）と動的に統合されています。`tsc --noEmit --allowJs` の出力がツール応答の一部として提供されます。AIアシスタントはこの出力を解析してコンパイラエラーを検出し、エラーが報告された場合は、書き込まれたコードに問題があることを示しているため、*さらなるアクションを続行すべきではありません*。
